    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = {
                        ...
                        Icon(Icons.Default.DeleteOutline, contentDescription = "Menu")
                    }
        ...
        content = { innerPadding ->
            Column(
                ...
                if (showClearAllDialog && userId != null) {
                    AlertDialog(
                        onDismissRequest = {
                            showClearAllDialog = false
                        },
                        title = { Text("Clear All Reminders") },
                        text = { Text("Are you sure you want to delete all reminders?") },
                        confirmButton = {
                            TextButton(onClick = {
                                val db = FirebaseFirestore.getInstance()
                                val reminderCollection = db.collection("users")
                                    .document(userId)
                                    .collection("reminders")

                                reminderCollection.get().addOnSuccessListener { snapshot ->
                                    val batch = db.batch()
